...
用户可见的账号安全页面只暴露账号级别的安全控制（更改密码、管理访问令牌、删除账户），不会展示任何已连接资源的原始凭证：

![Heicode Manager 的账号安全页面](/screenshots/account-security.jpg)
...
用户按准备清单连接资源
-> Heicode 接收授权结果
-> Heicode 后端写入 Azure Key Vault
-> Azure Key Vault 返回或形成 secret_ref
-> Heicode DB 保存 secret_ref
-> 前端只展示脱敏引用和状态
```

Heicode 接口不得返回明文密钥，写入时会校验 `secret_ref` 必须是 `azkv://` 格式，非法格式直接拒绝。日志、错误信息、审计摘要也不得包含明文密钥。

## 凭证生命周期

账号停用或组织关闭时，相关凭证会立即标记为已撤销，停止用于新任务；对应的 Key Vault 密钥材料会先软删除，超过保留期（默认 30 天）后不可恢复地清除。密钥支持轮换：轮换后 `secret_ref` 保持不变，只更新底层版本，不需要用户重新走一遍连接资源流程。
